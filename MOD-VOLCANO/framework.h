...
#define LOG_(...) { std::cout << "MOD-VOLCANO : " << std::format(__VA_ARGS__) << std::endl; AAAA << std::format(__VA_ARGS__) << std::endl; }

namespace Globals
{
    // MOD-VOLCANO Configuration
    std::string PlaylistName = "Playlist_DefaultSolo.Playlist_DefaultSolo";
    bool bLategame = true;
    
    // Bot System Configuration
    bool bBotSystemEnabled = true;
    int BotCount = 50; // Number of bots to spawn
