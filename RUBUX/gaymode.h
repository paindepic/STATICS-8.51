...
bool ReadyToStartMatchHook(AFortGameModeAthena* a1)
{
    ...
    if (!bPLAYLIST)
    {
        ...
        if (playlist)
        {
            ...
            Globals::NextTeamIndex = TeamIndex;
            Globals::NextBotTeamIndex = playlist->DefaultLastTeam;
    ...
    if (!bListneing)
    {
        ...
        {
            Bots::InitializeBotData();
            int MaxBots = Globals::MaxBotsToSpawn;
            if (a1->GameSession)
            {
                MaxBots = std::min(MaxBots, a1->GameSession->MaxPlayers - 1);
            }
