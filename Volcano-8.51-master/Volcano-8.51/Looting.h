...
int GetClipSize(UFortItemDefinition* ItemDef)
{
    if (auto RangedDef = Cast<UFortWeaponRangedItemDefinition>(ItemDef))
    {
        ...
        {
            auto RowMapPtr = (UE::TMap<FName, FFortRangedWeaponStats*>*)(__int64(DataTable) + 0x30);
            if (RowMapPtr)
            {
                for (int i = 0; i < RowMapPtr->Pairs.Elements.Data.Num(); ++i)
                {
                    auto& ElementData = RowMapPtr->Pairs.Elements.Data[i].ElementData;
...
void InitLooting()
{
    ...
    if (LootPackagesDataTable)
    {
        auto RowMapPtr = (UE::TMap<FName, FFortLootPackageData*>*)(__int64(LootPackagesDataTable) + 0x30);
        if (RowMapPtr)
        {
            for (int i = 0; i < RowMapPtr->Pairs.Elements.Data.Num(); ++i)
            {
                auto& CurrentRow = RowMapPtr->Pairs.Elements.Data[i];
