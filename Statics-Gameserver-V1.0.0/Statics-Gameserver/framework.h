...
static std::ofstream AAAA("StaticsGameserver.log");
#define LOG_(...) { std::cout << "STATICS GAMESERVER : " << std::format(__VA_ARGS__) << std::endl; AAAA << std::format(__VA_ARGS__) << std::endl; }
...
void Log(const std::string& msg)
{
	...
	if (firstCall)
	{
		std::ofstream logFile("StaticsGameserver_detailed.log", std::ios::trunc);
		if (logFile.is_open())
		{
			logFile << "[STATICS GAMESERVER]: Log file initialized!\n";
			logFile.close();
		}
		firstCall = false;
	}

	std::ofstream logFile("StaticsGameserver_detailed.log", std::ios::app);
	if (logFile.is_open())
	{
		logFile << "[STATICS GAMESERVER]: " << msg << std::endl;
		logFile.close();
	}

	std::cout << "[STATICS GAMESERVER]: " << msg << std::endl;
}
...
namespace Globals
{
	...
	int NextBotTeamIndex = 3;
	int CurrentBotsOnTeam = 0;
	int MaxPlayersPerTeam = 1;
}

// Building items for bots
static UFortBuildingItemDefinition* Floor = nullptr;
static UFortBuildingItemDefinition* Roof = nullptr;
static UFortBuildingItemDefinition* Stair = nullptr;
static UFortBuildingItemDefinition* Wall = nullptr;
static UFortEditToolItemDefinition* EditTool = nullptr;
...
static std::vector<UAthenaDanceItemDefinition*> Dances{};
...
// Bot-related function pointers
static void (*BotManagerSetup)(__int64 BotManager, __int64 Pawn, __int64 BehaviorTree, __int64 a4, DWORD* SkillLevel, __int64 a7, __int64 StartupInventory, __int64 BotNameSettings, __int64 a10, BYTE* CanRespawnOnDeath, unsigned __int8 BitFieldDataThing, BYTE* CustomSquadId) = decltype(BotManagerSetup)(GetOffsetBRUH(0x10EA060));
static void(*GiveAbilityOG)(UAbilitySystemComponent*, FGameplayAbilitySpecHandle*, FGameplayAbilitySpec) = decltype(GiveAbilityOG)(GetOffsetBRUH(0x7BC170));
static void (*AbilitySpecConstructor)(FGameplayAbilitySpec*, UGameplayAbility*, int, int, UObject*) = decltype(AbilitySpecConstructor)(GetOffsetBRUH(0x7DE360));
static bool (*InternalTryActivateAbility)(UAbilitySystemComponent* AbilitySystemComp, FGameplayAbilitySpecHandle AbilityToActivate, FPredictionKey InPredictionKey, UGameplayAbility** OutInstancedAbility, void* OnGameplayAbilityEndedDelegate, const FGameplayEventData* TriggerEventData) = decltype(InternalTryActivateAbility)(GetOffsetBRUH(0x7BC950));
static FGameplayAbilitySpecHandle(*GiveAbilityAndActivateOnce)(UAbilitySystemComponent* ASC, FGameplayAbilitySpecHandle*, FGameplayAbilitySpec) = decltype(GiveAbilityAndActivateOnce)(GetOffsetBRUH(0x7BC290));
...
void Listen()
{
	...
	LOG_("ReplicationDriver VTable : {}", __int64(repDriverVTable) - __int64(GetModuleHandleW(0)));
	ServerReplicateActors = decltype(ServerReplicateActors)(repDriverVTable[0x56]);
}
...
int GetPropOffset(UObject* Object, const std::string& PropertyName)
{
	for (auto Class = Object->Class; Class; Class = (UClass*)Class->Super)
	{
		...
		if (Property)
		{
			...
			{
				return *(int*)(__int64(Property) + 0x44);
			}
...
AFortPickupAthena* SpawnPickup(FFortItemEntry& ItemEntry, FVector Loc, EFortPickupSourceTypeFlag SourceType, EFortPickupSpawnSource Source, int OverrideCount = -1, AActor* OwningPawn = nullptr)
{
	...
	SpawnedPickup->TossPickup(Loc, OwningPawn, -1, true, SourceType, Source);
...
AFortPickupAthena* SpawnPickup(UFortItemDefinition* ItemDef, int OverrideCount, int LoadedAmmo, FVector Loc, EFortPickupSourceTypeFlag SourceType, EFortPickupSpawnSource Source, AActor* OwningPawn = nullptr)
{
	auto SpawnedPickup = SpawnActor<AFortPickupAthena>(AFortPickupAthena::StaticClass(), Loc);
	SpawnedPickup->bRandomRotation = true;
